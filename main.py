...
# Variables globales pour le suivi des sauts
...
#calcul de l'énergie cinétique du Hopper
...
#Calcul de l'énergie totale du Hopper
...
#Fonction servant à calculer la compression du ressort pour la commande verticale
...
def new_X(q, dq, hauteur, params):
    ...
    A = -w + l0 + X
    E_target = m1*g*(hauteur + r1) + m3*g*(hauteur + r2 + l0) #énergie à atteindre pour sauter à la hauteur désirée
    E_current = calculate_energy(q, dq, params) #énergie du Hopper
    delta_E = E_target - E_current #énergie à ajouter au Hopper pour atteindre l'énergie à atteindre pour sauter à la hauteur désirée
    ...
    if inside_sqrt < 0:
        ...
        return (np.sqrt(inside_sqrt) - A) #On donne de l'énergie au Hopper s'il ne saute pas assez haut
    else:
        return -(np.sqrt(inside_sqrt) - A) #On retire de l'énergie au Hopper s'il saute trop haut

    
#détection d'événement pour les transitions de phase
...
#Fonction servant à détecter le moment où la compression naturelle du ressort
#est maximale afin de pouvoir commencer la compression pour la commande verticale
def detection_sol(y):
...
def run(y, save_video=True, video_filename="simulation_hopper.mp4"):
    ...
    while running:
        ...
        if y0 > 0 and not en_vol:
            # Décollage
            en_vol = True
            y0_max = y0
        elif y0 > 0 and en_vol:
            # Toujours en vol
            if y0 > y0_max:
                y0_max = y0
        elif y0 <= 0 and en_vol:
            erreur = hauteur - y0_max
            erreurs.append(erreur)
            hauteurs_max.append(y0_max)
            if len(hauteurs_max) >= 2:
                h_i = hauteurs_max[-2]
                h_ip1 = hauteurs_max[-1]
                err_inter = 100 * (h_i - h_ip1) / h_i if h_i != 0 else 0
                erreurs_rel_inter.append(err_inter)
            hauteur_max_aff = hauteurs_max[8:]
            erreurs_rel_inter_aff = erreurs_rel_inter[8:]
            err_rel = 100 * erreur / hauteur
            erreurs_rel.append(err_rel)
            en_vol = False
    ...
    plt.figure(figsize=(10, 4))
    #plt.plot(range(1, len(erreurs)+1), erreurs, marker='o', linestyle='-')
    plt.plot(range(1, len(erreurs_rel)+1), erreurs_rel, marker='x', linestyle='-', label="Erreur relative (%)")
    plt.xlabel("Numéro du saut")
    plt.ylabel("Erreur relative")
    plt.title("Erreur sur la hauteur maximale du pied à chaque saut")
    plt.grid(True)
    plt.tight_layout()
    plt.show()
    
    plt.figure(figsize=(10, 4))
    plt.plot(range(2, len(hauteur_max_aff)+1), erreurs_rel_inter_aff, marker='s', linestyle='-', color='orange', label="Erreur relative inter-sauts (%)")
    plt.xlabel("Numéro du saut (i → i+1)")
    plt.ylabel("Erreur relative (%)")
    plt.title("Erreur relative inter-sauts (variation de hauteur max)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()
