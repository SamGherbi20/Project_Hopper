...
state_machine = OrderStateMachine() #importation de la machine à état

#définition des matrices du mouvement dérivées des équations d'Euler-Lagrange
...
def G_vector(q, dq, params):
    ...
    g = params.get("g", 9.81)
    k = params.get("k", 1000)
    K2 = params.get("K2", 1500)
    k_sol = params.get("k_sol", 500)
    l0 = params.get("l0", 0.4)
    X = params.get("X", 0.0)
    ...
    # Ressort conditionnel
    if l0 + X - w > 0:
        G[2] = (g * m3 * np.cos(theta1)
                - k * l0 - k * X + k * w
...
#Définition du vecteur couples moteurs + effets extérieurs
...
def control_forces(t, q, dq):
    ...
    gamma = np.zeros(n)
    gamma[4] = -50*dq[4]
    if l0 - q[2] + X < 0: #ici, on définit une butée pour le ressort de la jambe
        gamma[2] = -C_2*dq[2]
    return gamma

#résolution des équations d'Euler-Lagrange

def f(t,y):
    q = y[:n]   # Positions généralisées
    dq = y[n:]  # Vitesses généralisées
